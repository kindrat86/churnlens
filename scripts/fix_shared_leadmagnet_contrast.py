...
FIXES = [
    ...
    # 6-8. The block's <h2> and its lead line sit on the section's OWN
    # `linear-gradient(135deg,#f0f9ff,#e8f5e9)` and on the white card — both always
    # light, whatever the page does. So they need a pinned DARK colour, by exactly
    # the reasoning used for the cards above. Two ways they were wrong:
    #
    #   * `var(--ux-text,#1a1a1a)` (20 pages) — a page-following token on an
    #     always-light background. On the 148 dark pages it resolves to light text
    #     and the heading disappears: measured 1.03:1. This is the mirror of the
    #     glossary <dd> case — there the token is right because the text sits on the
    #     PAGE; here it is wrong because the text sits on the BLOCK.
    #   * `#e2e8f0` (15 heading + 2 lead lines) — light text applied wholesale by an
    #     earlier dark-mode pass. 1.03:1 and 1.23:1.
    #
    # Full attribute strings are matched, not the bare colour: `#e2e8f0` appears
    # legitimately hundreds of times elsewhere on these pages.
    ("lead-magnet <h2>: unpin page-following token on the block's light gradient",
     '<h2 style="font-size:1.5rem;margin-bottom:16px;color:var(--ux-text,#1a1a1a)"',
     '<h2 style="font-size:1.5rem;margin-bottom:16px;color:#1a1a1a"'),
    ("lead-magnet <h2>: light text on the block's light gradient 1.03 -> 15.8",
     '<h2 style="font-size:1.5rem;margin-bottom:16px;color:#e2e8f0"',
     '<h2 style="font-size:1.5rem;margin-bottom:16px;color:#1a1a1a"'),
    ("lead-magnet lead line: light text on the white card 1.23 -> 16.9",
     '<p style="font-size:1.1rem;font-weight:600;margin-bottom:8px;color:#e2e8f0;"',
     '<p style="font-size:1.1rem;font-weight:600;margin-bottom:8px;color:#1a1a1a;"'),
]
